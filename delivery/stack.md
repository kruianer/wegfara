...
- Auth: Passkey (WebAuthn) als einziger Regelweg. Seit req-066 startet
  die Geräte-Entsperrung beim Öffnen von selbst, ohne Knopfdruck; der
  Anmeldedialog erscheint erst, wenn sie scheitert, und trägt genau einen
  Weg — „Zugang verloren“ mit dem Magic Link ans hinterlegte Postfach.
  Notfallcodes und „Anderes Gerät verwenden“ gibt es nicht mehr. Keine
  Passwörter — kein Passwort-Feld, kein Hashing, kein Reset-Flow.
  Scheitert eine Anmeldung oder das Einrichten eines Passkeys, nennt die
  App den Grund und protokolliert den Schritt serverseitig
  (`lib/auth/passkey-fehler.ts`, `lib/auth/protokoll.ts`) — ein `catch`,
  der jeden Grund auf denselben Satz abbildet, ist nicht zulässig.
...
- KI: OpenAI API über das offizielle OpenAI-SDK. Modell:
  ...
  Account-Bezug; die POI-Suche per KI (req-014) und die KI-Planung
  (req-056) greifen nie darauf zurück.
...
| Bewertungsrunde    | Was der Reiseleiter im Planer über ausgewählte POIs einer Reise startet (req-054): solange sie läuft, gibt jeder Teilnehmer im Begleiter je POI seine Stimme ab. Zu einer Reise läuft höchstens eine. Beim Beenden bleiben die Stimmen erhalten und sichtbar; neue kommen nicht mehr dazu. Wer in einer laufenden Runde noch nicht überall gestimmt hat, hat eine „offene Bewertung“ und bleibt angemeldet (req-023). Ihren Stand zeigt der Bereich „Bewertungen“ des Planers (req-063); dort wird sie auch beendet.                                                                                                                            |
| Zustimmung         | Wie die Gruppe zu einem POI einer Bewertungsrunde steht (req-063): die gewichtete Summe seiner Stimmen — Will ich unbedingt = 2, Wäre schön = 1, Wenn wir Zeit haben = 0, Lieber nicht = −1, Ohne mich = −2. Sie ordnet die Zeilen im Bereich „Bewertungen“ des Planers, die höchste oben, und entscheidet nichts: aus den Stimmen folgt nie ein Status.                                                                                                                                                                                                                                                                                        |
...
| Reisetempo         | Was der Reiseleiter je Reise wählt (req-056): Entspannt, Ausgewogen (Vorgabe) oder Dicht. Es sagt, wie lang ein von der KI geplanter Tag höchstens dauert und wie viele POIs desselben Typs an einem Tag liegen dürfen — Restaurant und Hotel zählen nicht mit. Es wirkt ausschließlich auf die KI-Planung; von Hand verplant der Reiseleiter POIs weiterhin, wie er will.                                                                                                                                                                                                                                                                      |
| Planvorschlag      | Was „KI planen lassen“ ergibt (req-056): die auf die Reisetage verteilten POIs als Programmpunkte, im Zeitstrahl zur Ansicht und erkennbar als Vorschlag. Gespeichert wird er erst beim Übernehmen — dabei entstehen auch die Transfers dazwischen; „Verwerfen“ lässt den Plan unverändert.                                                                                                                                                                                                                                                                                                                                                     |
...
| Reiseleiter        | Die Rolle, die eine Reise führt. Jede Reise hat mindestens einen; der letzte lässt sich weder entfernen noch zum Teilnehmer herabstufen (req-021). Er bleibt unabhängig vom Zustand seiner Reisen angemeldet (req-023).                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Mein Bereich       | Die eine Stelle für alles, was zur angemeldeten Person und ihrem Account gehört (req-043): eine eigene Seite (`/mein-bereich`), aus dem Planer wie aus dem Begleiter erreichbar. Sie trägt untereinander die Karten „Meine Geräte“, „Personen“, „Einladungen“ und „Zugangsschlüssel“ — die letzten drei sieht ausschließlich ein Account-Admin. Die Karte „Notfallcodes“ stand bis req-066 daneben und ist mit den Codes entfallen. Sie ersetzt die bis dahin getrennten Bereiche „Konto“ (req-016), „Account“ (req-032) und „Nutzer“ (req-038). Nicht zu verwechseln mit der Account-Verwaltung des Gesamt-Admins, die alle Accounts betrifft. |
| Kostenplanung      | Was die Reise kosten wird — die Kalkulation **vorher** (req-062). Der Bereich „Kosten“ des Planers führt sie als Tabelle: je Programmpunkt eine Zeile, dazu manuelle Zeilen für alles ohne POI (Maut, Parkgebühren, Sprit), und unten die Gesamtkosten und die Kosten je Person. Preis je Person und Buchungsstatus stehen am POI (req-061) und sind hier nur mitbedienbar — eine zweite Kopie gibt es nicht. Nicht mit den Ausgaben zu verwechseln: die erfassen, was unterwegs tatsächlich gezahlt wurde.                                                                                                                                     |
